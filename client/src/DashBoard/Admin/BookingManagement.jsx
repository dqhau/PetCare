...
>>>>>>> 37b59e38b3e7f9b81d5a57ca12da94b2c3c5f2c5
      ...
      const response = await axios.put(`http://localhost:9999/booking/status/${id}`, 
        updateData,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
...
<<<<<<< HEAD
      // Sử dụng timeout dài hơn để đảm bảo server có đủ thời gian cập nhật dữ liệu
      setTimeout(() => {
        fetchBookings();
      }, 1000);
      ...
      if (viewBooking && viewBooking._id === id) {
        // Tải lại thông tin chi tiết từ server thay vì chỉ cập nhật trạng thái
        setTimeout(() => {
          handleViewBooking(id);
        }, 1000);
=======
      setTimeout(() => {
        fetchBookings();
...
<<<<<<< HEAD
        toast.current.show({ 
          severity: 'error', 
          summary: 'Lỗi', 
          detail: error.response?.data?.error || 'Không thể cập nhật trạng thái', 
          life: 3000 
        });
=======
