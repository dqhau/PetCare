...
const BookingManagement = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      console.log('Bookings data type:', typeof response.data);
      
      // Đảm bảo response.data luôn là mảng
      const bookingsData = Array.isArray(response.data) ? response.data : [];
      console.log(`Processed ${bookingsData.length} bookings`); 
      
      // Kiểm tra dữ liệu đầu tiên nếu có
      if (bookingsData.length > 0) {
        const firstBooking = bookingsData[0];
        console.log('First booking petId:', firstBooking.petId);
        console.log('First booking service:', firstBooking.service_type);
      }
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      ...
      // Nếu hủy đặt lịch, thêm lý do hủy
      let updateData = { order_status: status };
      
      if (status === 'Cancel') {
        // Thêm lý do hủy mặc định hoặc có thể hiển thị dialog yêu cầu nhập lý do
        updateData.cancel_reason = "Hủy bởi quản trị viên";
      }
      
      console.log(`Updating booking ${id} status to ${status}...`);
      
      // Gọi API cập nhật trạng thái - sử dụng API updateBookingStatusWithHistory thay vì updateBookingStatus
      const response = await axios.put(`http://localhost:9999/booking/update-status-with-history/${id}`, 
        updateData,
        {
          headers: {
  ...
  const fetchTimeslotsByDate = async (date) => {
    try {
      ...
      const response = await axios.get(`http://localhost:9999/timeslots?date=${formattedDate}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
        }
      });
  ...
  const handleChangeTimeslot = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:9999/booking/change-timeslot/${selectedBookingId}`,
