...
<<<<<<< HEAD
import { sendEmail, createBookingConfirmationEmail, createBookingApprovedEmail, createBookingCancelledEmail } from "../helpers/email_helper.js";
=======
...
<<<<<<< HEAD
    // 3. Xác định petId
    if (!petId) {
      return res.status(400).json({ error: "Missing petId. A pet must be selected for booking." });
    }
    
    // Kiểm tra pet có tồn tại và thuộc về user
...
<<<<<<< HEAD
    // Gửi email xác nhận đặt lịch cho khách hàng
    if (email) {
      try {
        ...
        if (completeBooking) {
          const emailContent = createBookingConfirmationEmail(
            completeBooking, 
            completeBooking.service_type, 
            completeBooking.timeslot
          );
          
          await sendEmail(
            email,
            `PetCare - Xác nhận đặt lịch dịch vụ ${completeBooking.service_type?.name || serviceName}`,
            emailContent
          );
          
          console.log(`Đã gửi email xác nhận đặt lịch đến ${email}`);
        } else {
          console.error('Không tìm thấy thông tin booking để gửi email');
        }
      } catch (emailError) {
        console.error('Lỗi khi gửi email xác nhận đặt lịch:', emailError);
...
<<<<<<< HEAD
    // Trả về kết quả ngay lập tức để không làm chậm trải nghiệm người dùng
    res.json(result);
    
    // Xử lý thông báo và email bất đồng bộ sau khi đã trả về kết quả
    if (result) {
      // Tạo thông báo và gửi email bất đồng bộ
      processNotificationAndEmail(result, req.body).catch(error => {
        console.error('Lỗi khi xử lý thông báo và email:', error);
      });
    }
=======
...
<<<<<<< HEAD
// Hàm xử lý thông báo và gửi email bất đồng bộ
async function processNotificationAndEmail(booking, requestBody) {
  try {
    // Lấy thông tin dịch vụ
    const serviceInfo = await Service.findById(booking.service_type);
    const serviceName = serviceInfo ? serviceInfo.name : 'Không xác định';
    ...
    // Tạo nội dung thông báo dựa trên trạng thái mới
    if (requestBody.order_status === "Processing") {
      message = `Lịch đặt dịch vụ ${serviceName} vào ngày ${bookingDate} lúc ${bookingTime}h của bạn đã được xác nhận`;
    } else if (requestBody.order_status === "Completed") {
      message = `Dịch vụ ${serviceName} vào ngày ${bookingDate} lúc ${bookingTime}h của bạn đã được hoàn thành`;
    } else if (requestBody.order_status === "Cancel") {
      const reason = requestBody.cancel_reason ? ` với lý do: ${requestBody.cancel_reason}` : '';
      message = `Lịch đặt dịch vụ ${serviceName} vào ngày ${bookingDate} lúc ${bookingTime}h của bạn đã bị hủy${reason}`;
    }
    
    // Tạo thông báo nếu có nội dung
    if (message) {
      // 1. Tạo và lưu thông báo cho user
      if (booking.userId) {
        const userNotification = new Notification({
          type: requestBody.order_status === "Cancel" ? 'cancellation' : 
                (requestBody.order_status === "Completed" ? 'completion' : 'booking'),
          message: message,
          relatedId: booking._id,
          userId: booking.userId // Thông báo cho user cụ thể
        });
        
        await userNotification.save();
      }
      
      // 2. Tạo và lưu thông báo cho admin (userId = null)
      // Tạo nội dung thông báo riêng cho admin
      let adminMessage = '';
      if (requestBody.order_status === "Processing") {
        adminMessage = `Xác nhận đặt lịch: Lịch dịch vụ ${serviceName} vào ngày ${bookingDate} lúc ${bookingTime}h của ${booking.customer_name} đã được xác nhận`;
      } else if (requestBody.order_status === "Completed") {
        adminMessage = `Hoàn thành dịch vụ: Dịch vụ ${serviceName} vào ngày ${bookingDate} lúc ${bookingTime}h của ${booking.customer_name} đã được hoàn thành`;
      } else if (requestBody.order_status === "Cancel") {
        const reason = requestBody.cancel_reason ? ` với lý do: ${requestBody.cancel_reason}` : '';
        adminMessage = `Hủy đặt lịch: ${booking.customer_name} đã hủy lịch dịch vụ ${serviceName} vào ngày ${bookingDate} lúc ${bookingTime}h${reason}`;
      } else if (requestBody.order_status === "Pending") {
        adminMessage = `Đặt lịch mới: ${booking.customer_name} đã đặt lịch dịch vụ ${serviceName} vào ngày ${bookingDate} lúc ${bookingTime}h`;
      }
      ...
      if (adminMessage) {
        ...
        console.log('Đã gửi thông báo cho admin về cập nhật trạng thái booking');
      }
      
      // 2. Gửi email nếu có email
      if (booking.email) {
        // Lấy thông tin đầy đủ từ database (chỉ cần lấy một lần)
        ...
        if (completeBooking) {
          let emailContent = '';
          let emailSubject = '';
          const bookingServiceName = completeBooking.service_type?.name || serviceName;
          
          // Tạo nội dung email dựa trên trạng thái mới
          if (requestBody.order_status === "Processing") {
            emailSubject = `PetCare - Đơn đặt lịch dịch vụ ${bookingServiceName} đã được xác nhận`;
            emailContent = createBookingApprovedEmail(
              completeBooking, 
              completeBooking.service_type, 
              completeBooking.timeslot
            );
          } else if (requestBody.order_status === "Cancel") {
            emailSubject = `PetCare - Đơn đặt lịch dịch vụ ${bookingServiceName} đã bị hủy`;
            emailContent = createBookingCancelledEmail(
              completeBooking, 
              completeBooking.service_type, 
              completeBooking.timeslot, 
              requestBody.cancel_reason
            );
          }
          
          // Chỉ gửi email khi có nội dung
          if (emailContent) {
            // Gửi email bất đồng bộ và bỏ qua lỗi nếu có
            sendEmail(
              completeBooking.email,
              emailSubject,
              emailContent
            ).then(() => {
              console.log(`Đã gửi email thông báo trạng thái đơn hàng đến ${completeBooking.email}`);
            }).catch(emailError => {
              console.error('Lỗi khi gửi email thông báo trạng thái đơn hàng:', emailError);
            });
          }
        } else {
          console.error('Không tìm thấy thông tin booking để gửi email');
    ...
    console.error('Lỗi trong quá trình xử lý thông báo và email:', error);
